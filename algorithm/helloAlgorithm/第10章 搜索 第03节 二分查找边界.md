...
实际上，我们可以利用查找最左元素的函数来查找最右元素，具体方法为：
**将查找最右一个 `target` 转化为查找最左一个 `target + 1`**。

如下图所示，查找完成后，指针 i 指向最左一个 `target + 1`（如果存在），
而 j 指向最右一个 `target` ，**因此返回 j 即可**。

![将查找右边界转化为查找左边界](img/10/03/binary_search_right_edge_by_left_edge.png)
...
![将查找边界转化为查找元素](img/10/03/binary_search_edge_by_element.png)
